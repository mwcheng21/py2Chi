...
def main(fileContent, filename):
    ...
    if fileExtension == ".cryst" or fileExtension == ".pfam":
        ...
        tryOpen(str(filename[:-len(str(fileExtension))]) + ".py")
...
def tryOpen(openFile):
    try:
        ChimeraPath = r'C:\Program Files\Chimera 1.10.1\bin\chimera.exe'
        subprocess.Popen("%s %s" % (ChimeraPath, openFile))
    except WindowsError:
        try:
            ChimeraPath = r'C:\Program Files\Chimera 1.10.2\bin\chimera.exe'
            subprocess.Popen("%s %s" % (ChimeraPath, openFile))
        except:
            try:
                ChimeraPath = r'C:\Program Files\Chimera 1.11.1\bin\chimera.exe'
                subprocess.Popen("%s %s" % (ChimeraPath, openFile))
            except:
                try:
                    ChimeraPath = r'C:\Program Files\Chimera 1.11.2\bin\chimera.exe'
                    subprocess.Popen("%s %s" % (ChimeraPath, openFile))
                except:
                    print 'Cannot find chimera. Please make sure you have versions 1.10.1 - 1.11.2 and it is in C:\\Program Files'
