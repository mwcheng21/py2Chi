...
def sele(strInfo, iterate):
    ...
    if findWord('chain', strInfo) > 0:
        ...
        length = retLine[:findWord("s", retLine)]
        retLine = retLine[findWord("s", retLine):]
    if findWord(', het', strInfo) >0:
        retLine = "select " + ":/isHet"  #:/isHet
        length = 2
   #retLine = retLine + "\r\nwrite selected relative #0 #" + WHAT GOES HERE + " "+ getFilePath()[:-len(str(fileString))] + name + ".pdb"
    retLine = retLine + "\r\nwrite selected relative #0 #16" + " " + getFilePath()[:-len(str(fileString))] + name + ".pdb"
    retLine = retLine + "\r\nopen " + name + ".pdb\r\n" + "select #" + str(iterate) + "-" ##DO I NEED THIS -->+ str(iterate + length) + "\r\nrepr sphere selected\r\n\r\n"
    print retLine
    return retLine
