...
def hide(strInfo, iterate):
    ...
    if findWord("chain", strInfo) > 0:
        ...
        if findWord("cartoon", strInfo) >0:
            ...
            if findWord("domain", fileString) > -1:
                retLine = "~ribbon #" + str(iterate) + " & ~" + str(atomSpec) + "\r\n"
                retLine = retLine + "~display #" + str(iterate)  + " & ~" + str(atomSpec) + "\r\n"
            else:
                retLine = "sel #" + str(iterate) + "" + str(atomSpec) + "\r\n"
                retLine = retLine + "sel invert sel\r\ndelete sel\r\n"
...
def show(strInfo, iterate):
    ...
    if findWord("cartoon", strInfo) > 0:
        retLine = ""
        if findWord("chain", strInfo) > 0:
            retLine = "sel #" + str(iterate) + ":." + strInfo[findWord("chain", strInfo)+6:findWord("chain", strInfo)+7].lower() + "\r\n"
            retLine = retLine + "sel invert sel\r\n"
            retLine = retLine + "~display sel\r\n~ribbon sel\r\n"
    else:
...
def spec(strInfo, iterate):
    ...
    if len(strInfo2) > 1:
        ...
        strInfo2 = strInfo3
        if iterate > -1 and findWord("resi", saveStr) > 0 and findWord("domain", fileString) > -1:
            strInfo2 = strInfo2[7:]
