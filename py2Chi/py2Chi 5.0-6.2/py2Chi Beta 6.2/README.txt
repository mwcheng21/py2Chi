...
---------------------
 ...
 B. Limitations
   1. Unlike pyMol, Chimera does not allow a subset of a chain to be colored. In pyMol, some scripts will rainbow a chain by domain. In chimera, the cooresponding representation is coloring the domain by a random color
   2. ProtCID pyMol scripts select ligands as separate objects. Chimera cannot select certain groups of ligands and place them in the model panel so the converted Chimera scripts place them as separate selectable objects. Run the command namesel to see a list of specified Ligands.
----------------------
